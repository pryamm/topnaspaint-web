...
export function HeroSection() {
  ...
  return (
    <section id="home" className="relative pt-24 pb-16 px-4 md:px-12 max-w-7xl mx-auto overflow-hidden">
      ...
      <div className="relative z-10 mb-20 flex flex-col items-center text-center max-w-4xl mx-auto">
        ...
        <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/60 backdrop-blur-md border border-white/50 shadow-sm text-sm font-semibold text-emerald-800 mb-8 hover:scale-105 transition-transform cursor-default">
          ...
          <span>Topnas Paint: Pabrikan Cat Kualitas Premium Bersertifikat SNI</span>
        </div>
        ...
        <p className="text-xl text-slate-600 max-w-2xl mb-10 leading-relaxed">
          Tingkatkan margin keuntungan toko bangunan Anda dengan produk cat berdaya tutup tinggi, dukungan pemasaran penuh, dan suplai yang konsisten.
        </p>
        
        {/* Centered Actions */}
        <div className="flex flex-wrap justify-center gap-4">
          <a 
            href="https://wa.me/6281234567890?text=Halo%20Topnas%20Paint,%20saya%20tertarik%20menjadi%20mitra%20distributor." 
            target="_blank"
      ...
      <div className="relative z-10 mt-16">
        <h3 className="text-center text-sm font-bold text-slate-400 uppercase tracking-widest mb-6">Program & Promo Spesial</h3>
        <Carousel>
          {promos.map((promo, idx) => (
            <div key={idx} className="relative rounded-[2.5rem] overflow-hidden h-[22rem] md:h-[28rem] shadow-[0_20px_40px_rgb(0,0,0,0.08)] group hover:-translate-y-1 transition-all duration-500 border border-white/40 bg-white">
              
              {/* Image Container */}
              <div className="absolute inset-0 z-0">
                <Image src={promo.image} alt={promo.title} fill sizes="(max-width: 768px) 100vw, 33vw" priority={idx === 0} className="object-cover group-hover:scale-105 transition-transform duration-700 ease-out" />
              ...
              <div className="absolute bottom-4 left-4 right-4 z-20">
                <div className="bg-white/10 backdrop-blur-xl border border-white/20 p-6 rounded-[2rem] shadow-2xl">
                  ...
                  <h3 className="text-2xl md:text-3xl font-extrabold text-white mb-2 leading-tight drop-shadow-md">{promo.title}</h3>
                  <p className="text-white/90 text-sm md:text-base leading-relaxed drop-shadow-sm max-w-sm">{promo.desc}</p>
                </div>
              </div>
